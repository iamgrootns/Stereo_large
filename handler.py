...
try:
    ...
    print("Loading facebook/musicgen-stereo-large...")
    device = "cuda:0" if torch.cuda.is_available() else "cpu"
    print(f"Using device: {device}")

    # --- MODEL CHANGE HERE ---
    # Load the LARGE stereo model using the transformers pipeline
    synth = pipeline(
        "text-to-audio",
        "facebook/musicgen-stereo-large", # Changed from medium to large
        device=device,
...
if __name__ == "__main__":
    print("Starting RunPod serverless worker for large stereo model...")
